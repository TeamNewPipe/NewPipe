...
object SparseItemUtil {
    ...
    fun fetchItemInfoIfSparse(context: Context,
                              item: StreamInfoItem,
                              callback: java.util.function.Consumer<SinglePlayQueue?>) {
        if (((StreamTypeUtil.isLiveStream(item.getStreamType()) || item.getDuration() >= 0)
                        && !Utils.isNullOrEmpty(item.getUploaderUrl()))) {
        ...
        fetchStreamInfoAndSaveToDatabase(context, item.getServiceId(), item.getUrl(),
                java.util.function.Consumer({ streamInfo: StreamInfo? -> callback.accept(SinglePlayQueue(streamInfo)) }))
    }
    ...
    fun fetchUploaderUrlIfSparse(context: Context,
        ...
        fetchStreamInfoAndSaveToDatabase(context, serviceId, url,
                java.util.function.Consumer({ streamInfo: StreamInfo -> callback.accept(streamInfo.getUploaderUrl()) }))
    }
    ...
    fun fetchStreamInfoAndSaveToDatabase(context: Context,
                                         serviceId: Int,
                                         url: String,
                                         callback: java.util.function.Consumer<StreamInfo>) {
                ...
                .subscribe(io.reactivex.rxjava3.functions.Consumer({ result: StreamInfo ->
                    ...
                    Completable.fromAction(Action({
                        NewPipeDatabase.getInstance(context)
                            ...
                            .doOnError(io.reactivex.rxjava3.functions.Consumer({ throwable: Throwable? ->
