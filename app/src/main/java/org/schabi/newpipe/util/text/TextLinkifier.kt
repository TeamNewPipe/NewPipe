...
object TextLinkifier {
    private val TAG = TextLinkifier::class.java.simpleName
    ...
    @JvmField
    val SET_LINK_MOVEMENT_METHOD = Consumer<TextView> { v ->
        v.movementMethod = LongPressLinkMovementMethod.getInstance()
    }
    ...
    fun fromDescription(
        ...
        when (description.type()) {
            Description.Type.HTML -> fromHtml(
                textView, description.content(), htmlCompatFlag,
                relatedInfoService, relatedStreamUrl, onCompletion
            )
            Description.Type.MARKDOWN -> fromMarkdown(
                textView, description.content(),
                relatedInfoService, relatedStreamUrl, onCompletion
            )
            Description.Type.PLAIN_TEXT -> fromPlainText(
                textView, description.content(),
                relatedInfoService, relatedStreamUrl, onCompletion
            )
        }
     ...
     *
     * This method will call [changeLinkIntents] after having linked the URLs with
     * [TextView.setAutoLinkMask] and [TextView.setText].
     *
    ...
    fun fromPlainText(
        ...
        changeLinkIntents(
            textView, textView.text, relatedInfoService,
            relatedStreamUrl, onCompletion
        )
    }
    ...
    fun fromMarkdown(
        ...
        changeLinkIntents(
            textView, markwon.toMarkdown(markdownBlock),
            relatedInfoService, relatedStreamUrl, onCompletion
        )
    }
     ...
     * [addClickListenersOnTimestamps] method and click listeners on hashtags, by
     * using [addClickListenersOnHashtags], which will open a search on the current service with the hashtag.
     *
    ...
    private fun changeLinkIntents(
        ...
        CoroutineScope(Dispatchers.Default).launch {
            try {
                ...
                for (span in urls) {
                    val url = span.url
                    val longPressClickableSpan = UrlLongPressClickableSpan(context, url)

                    textBlockLinked.setSpan(
                ...
                relatedInfoService?.let { service ->
                    relatedStreamUrl?.let { url ->
                        addClickListenersOnTimestamps(
                            context,
                            textBlockLinked,
                            service,
                            url
                    ...
                    addClickListenersOnHashtags(context, textBlockLinked, service)
                }
     ...
     *
     * This method finds all hashtags in the [SpannableStringBuilder] of the description
    ...
    private fun addClickListenersOnHashtags(
        ...
        while (hashtagsMatches.find()) {
            ...
            if (spannableDescription.getSpans(
    ...
    private fun addClickListenersOnTimestamps(
        ...
        while (timestampsMatches.find()) {
            val timestampMatchDTO = TimestampExtractor.getTimestampFromMatcher(
                timestampsMatches, descriptionText
            ) ?: continue

            spannableDescription.setSpan(
                TimestampLongPressClickableSpan(
                    context, descriptionText, relatedInfoService, relatedStreamUrl, timestampMatchDTO
                ),
    ...
    private fun setTextViewCharSequence(
        textView: TextView,
