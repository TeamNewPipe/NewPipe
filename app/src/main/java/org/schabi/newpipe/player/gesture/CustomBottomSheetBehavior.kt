...
class CustomBottomSheetBehavior(context: Context,
                                attrs: AttributeSet?) : BottomSheetBehavior<FrameLayout>(context, attrs) {
    var globalRect: Rect = Rect()
    private var skippingInterception: Boolean = false
    ...
    public override fun onInterceptTouchEvent(parent: CoordinatorLayout,
                                              child: FrameLayout,
        ...
        if (skippingInterception || getState() == STATE_SETTLING) {
            return false
        }
        ...
        if ((getState() == STATE_EXPANDED
            ...
            for (element: Int in skipInterceptionOfElements) {
                val view: View? = child.findViewById(element)
                if (view != null) {
                    val visible: Boolean = view.getGlobalVisibleRect(globalRect)
                    if ((visible
                                    && globalRect.contains(event.getRawX().toInt(), event.getRawY().toInt()))) {
