 ...
 * [androidx.media3.datasource.DefaultHttpDataSource], for YouTube streams.
 *
 ...
 *
 * There are many unused methods in this class because everything was copied from [androidx.media3.datasource.DefaultHttpDataSource] with as little changes as possible.
 * SonarQube warnings were also suppressed for the same reason.
 */
@Suppress("squid:S3011", "squid:S4738")
class YoutubeHttpDataSource private constructor(
    ...
    class Factory : HttpDataSource.Factory {
        ...
        private var connectTimeoutMs = DefaultHttpDataSource.DEFAULT_CONNECT_TIMEOUT_MILLIS
        private var readTimeoutMs = DefaultHttpDataSource.DEFAULT_READ_TIMEOUT_MILLIS
         ...
         *
         * Note that it must be not enabled on streams which are using a [androidx.media3.exoplayer.source.ProgressiveMediaSource], as it will break playback
    ...
    override fun getResponseHeaders(): Map<String, List<String>> {
        val currentConnection = connection ?: return ImmutableMap.of()
        return NullFilteringHeadersMap(currentConnection.headerFields)
    }
    ...
    override fun open(dataSpecParameter: DataSpec): Long {
        ...
        try {
            var stream: InputStream = httpURLConnection.inputStream
            if (isCompressed) {
        ...
        try {
            skipFully(bytesToSkip, dataSpecParameter)
        } catch (e: IOException) {
            closeConnectionQuietly()
    ...
    private fun makeConnection(
        ...
        defaultRequestProperties?.let { requestHeaders.putAll(it.snapshot) }
    ...
    @Throws(IOException::class)
    private fun skipFully(bytesToSkipParameter: Long, dataSpecToUse: DataSpec) {
        var bytesToSkip = bytesToSkipParameter
        if (bytesToSkip == 0L) {
            return
        }
        ...
        while (bytesToSkip > 0) {
            ...
            val read = castNonNull(inputStream).read(skipBuffer, 0, readLength)
    ...
    @Throws(IOException::class)
    private fun readInternal(buffer: ByteArray, offset: Int, readLengthParameter: Int): Int {
        var readLength = readLengthParameter
        if (readLength == 0) {
            return 0
        }
        ...
        val read = castNonNull(inputStream).read(buffer, offset, readLength)
        if (read == -1) {
            return C.RESULT_END_OF_INPUT
        }
    ...
    private class NullFilteringHeadersMap(
            ...
            get() = headers.entries
                ...
                .map { entry ->
                    object : Map.Entry<String, List<String>> {
                        ...
                        override val value: List<String> = entry.value
                    }
                }
                .toSet()

        override val keys: Set<String>
            get() = headers.keys.filterNotNull().toSet()

        override val size: Int
            get() = headers.size - if (headers.containsKey(null)) 1 else 0

        override val values: Collection<List<String>>
            get() = headers.entries.filter { it.key != null }.map { it.value }

        override fun containsKey(key: String): Boolean = headers.containsKey(key)

        override fun containsValue(value: List<String>): Boolean = headers.containsValue(value)

        override fun get(key: String): List<String>? = headers[key]

        override fun isEmpty(): Boolean = size == 0
    }

    companion object {
        private val TAG = YoutubeHttpDataSource::class.java.simpleName
         ...
         */
        private fun maybeTerminateInputStream(
            ...
            try {
                ...
                if ("com.android.okhttp.internal.http.HttpTransport\$ChunkedInputStream" == className ||
                    ...
                    val unexpectedEndOfInput = checkNotNull(superclass).getDeclaredMethod(
