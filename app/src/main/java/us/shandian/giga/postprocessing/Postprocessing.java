...
public abstract class Postprocessing implements Serializable {
    ...
    private static final long MAX_COVER_ART_BYTES = 8 * 1024 * 1024; // 8 MiB
     ...
     *
     * @return the cover art bytes, or {@code null} if none could be obtained
     */
    byte[] fetchCoverArt() {
        ...
        try {
            ...
            try (final Response response = DownloaderImpl.getInstance().getClient()
                ...
                final byte[] cover = response.body().bytes();
                return cover.length > MAX_COVER_ART_BYTES ? null : cover;
            }
