            ...
            case 'a':
                for (AudioStream audio : mExtractor.getAudioStreams()) {
                    if (audio.average_bitrate == mRecovery.desiredBitrate && audio.getFormat() == mRecovery.format
                            && audio.getDeliveryFormat() instanceof DeliveryFormat.Direct) {
                        url = ((DeliveryFormat.Direct) audio.getDeliveryFormat()).getUrl();
                        break;
                    }
                }
                break;
            case 'v':
                ...
                for (VideoStream video : videoStreams) {
                    if (video.resolution.equals(mRecovery.desired) && video.getFormat() == mRecovery.format
                            && video.getDeliveryFormat() instanceof DeliveryFormat.Direct) {
                        url = ((DeliveryFormat.Direct) video.getDeliveryFormat()).getUrl();
                        break;
                    }
                }
                break;
            case 's':
                for (SubtitlesStream subtitles : mExtractor.getSubtitles(mRecovery.format)) {
                    ...
                    if (tag.equals(mRecovery.desired) && subtitles.isAutoGenerated() == mRecovery.desired2
                            && subtitles.getDeliveryFormat() instanceof DeliveryFormat.Direct) {
                        url = ((DeliveryFormat.Direct) subtitles.getDeliveryFormat()).getUrl();
                        break;
                    }
                }
                break;
