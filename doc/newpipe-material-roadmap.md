...
- The progress strip is lifted inside the rounded thumbnail, and stream holders use a larger raised duration/time badge margin only while visible watched progress is present; unwatched items keep the normal lower badge position and playback/loading/watch-history behavior is unchanged.
